...
class AnalysisService:
    ...
    def executor(self) -> ThreadPoolExecutor:
        ...
        if self._executor is None:
            self._executor = ThreadPoolExecutor(max_workers=self._max_workers, thread_name_prefix="analysis_")
        return self._executor

    def submit_analysis(self, code: str, report_type: Union[ReportType, str] = ReportType.SIMPLE) -> Dict[str, Any]:
        """
        提交异步分析任务
        ...
        logger.info(
            f"[AnalysisService] 已提交股票 {code} 的分析任务, task_id={task_id}, report_type={report_type.value}"
        )
    ...
    def _run_analysis(self, code: str, task_id: str, report_type: ReportType = ReportType.SIMPLE) -> Dict[str, Any]:
        """
        执行单只股票分析
        ...
        try:
            ...
            from core.services.analysis.stock_report.pipeline import StockAnalysisPipeline
            ...
            result = pipeline.process_single_stock(
                code=code, skip_analysis=False, single_stock_notify=True, report_type=report_type
            )
            ...
            if result:
                ...
                with self._tasks_lock:
                    self._tasks[task_id].update(
                        {"status": "completed", "end_time": datetime.now().isoformat(), "result": result_data}
                    )
                ...
                with self._tasks_lock:
                    self._tasks[task_id].update(
                        {"status": "failed", "end_time": datetime.now().isoformat(), "error": "分析返回空结果"}
                    )
            ...
            with self._tasks_lock:
                self._tasks[task_id].update(
                    {"status": "failed", "end_time": datetime.now().isoformat(), "error": error_msg}
                )
