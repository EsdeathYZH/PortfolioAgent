    ...
    try:
        ...
        if recent:
            ...
            for row in recent:
                ...
                print(
                    f"  {row[0]:<10} {row[1]!s:<12} {row[2]:<10.2f} {row[3]:<8.2f} {vol_str:<15} {row[5] or 'Unknown'}"
                )
    finally:
        session.close()
...
def test_llm():
    ...
    try:
        ...
        print(
            f"  技术分析: {result.technical_analysis[:80]}..."
            if len(result.technical_analysis) > 80
            else f"  技术分析: {result.technical_analysis}"
        )
        print(
            f"  消息面: {result.news_summary[:80]}..."
            if len(result.news_summary) > 80
            else f"  消息面: {result.news_summary}"
        )
...
def test_notification():
    ...
    if service.is_available():
        ...
        webhook_preview = (
            config.wechat_webhook_url[:50] + "..." if len(config.wechat_webhook_url) > 50 else config.wechat_webhook_url
        )
...
def query_stock_data(stock_code: str, days: int = 10):
    ...
    try:
        ...
        if rows:
            print(f"\n  最近 {len(rows)} 条记录:\n")
            print(
                f"  {'日期':<12} {'开盘':<10} {'最高':<10} {'最低':<10} {'收盘':<10} {'涨跌%':<8} {'MA5':<10} {'MA10':<10} {'量比':<8}"
            )
            print("  " + "-" * 100)
            for row in rows:
                dt, open_, high, low, close, pct_chg, vol, amt, ma5, ma10, ma20, vol_ratio = row
                print(
                    f"  {dt!s:<12} {open_:<10.2f} {high:<10.2f} {low:<10.2f} {close:<10.2f} {pct_chg:<8.2f} {ma5:<10.2f} {ma10:<10.2f} {vol_ratio:<8.2f}"
                )
        else:
