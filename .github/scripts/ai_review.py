...
def get_changed_files():
    ...
    result = subprocess.run(
        ["git", "diff", "--name-only", f"origin/{base_ref}...HEAD", "--", "*.py"], capture_output=True, text=True
    )
...
def review_with_openai(prompt):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=model, messages=[{"role": "user", "content": prompt}], max_tokens=2000, temperature=0.3
        )
