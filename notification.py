...
class NotificationService:
    ...
    def __init__(self):
        ...
        self._serverchan_config = {
            "send_key": getattr(config, "serverchan_send_key", None),
    ...
    def _is_serverchan_configured(self) -> bool:
        """检查 ServerChan 配置是否完整"""
        return bool(self._serverchan_config["send_key"])
    ...
    def _send_pushover_message(
        self, api_url: str, user_key: str, api_token: str, message: str, title: str, priority: int = 0
    ) -> bool:
        """
        发送单条 Pushover 消息
    ...
    def _send_pushover_chunked(
        self, api_url: str, user_key: str, api_token: str, content: str, title: str, max_length: int
    ) -> bool:
        """
        分段发送长 Pushover 消息
    ...
    def send_to_serverchan(self, content: str, title: Optional[str] = None) -> bool:
        ...
        Returns:
            是否发送成功
        """
        if not self._is_serverchan_configured():
            logger.warning("Server酱配置不完整，跳过推送")
            return False

        send_key = self._serverchan_config["send_key"]
        channel = self._serverchan_config.get("channel")
        noip = self._serverchan_config.get("noip", False)
        ...
        # Server酱 desp 最大长度 32KB（32768字节）
        max_desp_bytes = 32 * 1024  # 32768 字节

        # 检查内容长度
        content_bytes = len(content.encode("utf-8"))
        if content_bytes > max_desp_bytes:
            logger.info(f"Server酱消息内容超长({content_bytes}字节)，将分批发送")
            return self._send_serverchan_chunked(api_url, content, title, channel, noip, max_desp_bytes)

        # 单条消息发送
        return self._send_serverchan_message(api_url, content, title, channel, noip)

    def _send_serverchan_message(
        self, api_url: str, content: str, title: str, channel: Optional[str], noip: bool
    ) -> bool:
        """
        发送单条 Server酱消息
    ...
    def _send_serverchan_chunked(
        self, api_url: str, content: str, title: str, channel: Optional[str], noip: bool, max_bytes: int
    ) -> bool:
        """
        分批发送长 Server酱消息
        ...
        if "\n---\n" in content:
            ...
            # 无法智能分割，按行强制分割
            return self._send_serverchan_force_chunked(api_url, content, title, channel, noip, max_bytes)
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_title = f"{title} ({i+1}/{total_chunks})" if total_chunks > 1 else title
            # 确保标题不超过32字符
            if len(chunk_title) > 32:
                chunk_title = chunk_title[:29] + "..."

            try:
                if self._send_serverchan_message(api_url, chunk, chunk_title, channel, noip):
                    success_count += 1
                    logger.info(f"Server酱第 {i+1}/{total_chunks} 批发送成功")
                else:
                    logger.error(f"Server酱第 {i+1}/{total_chunks} 批发送失败")
            except Exception as e:
                logger.error(f"Server酱第 {i+1}/{total_chunks} 批发送异常: {e}")
    ...
    def _send_serverchan_force_chunked(
        self, api_url: str, content: str, title: str, channel: Optional[str], noip: bool, max_bytes: int
    ) -> bool:
        """
        强制按字节分割发送（无法智能分割时的 fallback）
    ...
    def _build_custom_webhook_payload(self, url: str, content: str) -> dict:
        ...
        if "api.day.app" in url_lower:
            return {"title": "股票分析报告", "body": content[:4000], "group": "stock"}  # Bark 限制
