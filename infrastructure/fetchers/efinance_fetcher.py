...
# 缓存实时行情数据（避免重复请求）
_realtime_cache: Dict[str, Any] = {"data": None, "timestamp": 0, "ttl": 60}  # 60秒缓存有效期
...
class EfinanceFetcher(BaseFetcher):
    ...
    def _fetch_stock_data(self, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            df = ef.stock.get_quote_history(
                stock_codes=stock_code, beg=beg_date, end=end_date_fmt, klt=101, fqt=1  # 日线  # 前复权
            )
    ...
    def _fetch_etf_data(self, stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        logger.info(
            f"[API调用] ef.fund.get_quote_history(fund_code={stock_code}, "
            f"beg={beg_date}, end={end_date_fmt}, klt=101, fqt=1)"
        )
        ...
        try:
            ...
            df = ef.fund.get_quote_history(
                fund_code=stock_code, beg=beg_date, end=end_date_fmt, klt=101, fqt=1  # 日线  # 前复权
            )
